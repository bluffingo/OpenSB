...
if (result("SELECT COUNT(rating) FROM rating WHERE video=? AND user=?", [result("SELECT id FROM videos WHERE video_id=?", [$_POST['vidid']]),$currentUser['id']]) != 0) {
	query("DELETE FROM rating WHERE user=? AND video=?",
		[$currentUser['id'],result("SELECT id FROM videos WHERE video_id=?",[$_POST['vidid']])]);
	echo 0;
} else {	
	query("INSERT INTO rating (user, video, rating) VALUES (?,?,?)",
		[$currentUser['id'],result("SELECT id FROM videos WHERE video_id=?",[$_POST['vidid']]),$_POST['rating']]);
	echo 1;
}
