...
if (isset($_POST['upload']) and isset($userdata['name'])) {
	...
	$target_file = 'image/'.$new.'.'.$ext;
	if (move_uploaded_file($temp_name, $target_file)){
		query("INSERT INTO image (image_id, title, author, time, file) VALUES (?,?,?,?,?)",
			[$new,$title,$userdata['id'],time(),'image/'.$new.'.'.$ext]);

		redirect('./view.php?i='.$new);
	}
}

$twig = twigloader();
echo $twig->render('upload_image.twig');